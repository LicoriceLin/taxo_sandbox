...
if __name__=='__main__':
    ...
    prev_run='v1_240521-210442_seed77'
    device=1
    batch_size=2
    batch_size_val=25
    max_domain=15
    ...
    odir=f'train/{prev_run}' #TODO odir is 
    reader=SummaryReader(odir+'/log')
    scalars=reader.scalars
    global_step=int(scalars['step'].max())
    prev_epoc=int(scalars[scalars['tag']=='epoch']['value'].max())
    seed=int(prev_run.split('_')[-1].replace('seed',''))
    # print(seed,prev_epoc,global_step)
    ...
    try:
        hierar_esmmodel.load_state_dict(torch.load(f'train/{prev_run}/ep-{prev_epoc}.pt'))
    except:
        prev_epoc=prev_epoc-1
        hierar_esmmodel.load_state_dict(torch.load(f'train/{prev_run}/ep-{prev_epoc}.pt'))
    ...
    writer = SummaryWriter(log_dir=odir+'/log')
    # %% train
    ...
    best_val_acc=0
    # import pdb;pdb.set_trace()
    for epoch in range(prev_epoc+1,prev_epoc+31):
