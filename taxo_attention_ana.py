...
embedding_layer = hierar_esmmodel.backbone.embeddings.word_embeddings
...
batch_name,domains,batch_y,domains_mask=process_batch(dataset.fetch_single(entry_id),hierar_esmmodel.max_domain)
...
def forward_func(inputs_embeds:torch.Tensor,attention_mask:torch.Tensor,domains_mask:torch.Tensor):
    return hierar_esmmodel({'inputs_embeds':inputs_embeds,'attention_mask':attention_mask},domains_mask)[-1]
